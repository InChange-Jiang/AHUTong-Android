...
- 每个新的合格 decision 都可以展示，同一时间仍只显示一个；不设置跨 decision 的最小时间间隔、session 次数或每日次数上限。
- 普通关闭、自动超时、路由切换和忽略只撤销当前气泡，不写负样本、不改变模型分数、晋级证据或后续展示资格，也不设置 action 冷却；只有用户明确选择单 action“30 天内不再推荐”时才执行该偏好。
...
- 支持图标、功能名、简短本地 reason code、点击、关闭、超时和 TalkBack。
- 不遮挡底栏、主要按钮、编辑浮层、支付页和付款码。
- 当前路由变化、切后台或开关关闭时立即撤销。
- 点击通过 `AppActionDispatcher` 进入同一受控业务路径，source 固定为 `SUGGESTION`。
- 点击必须先耐久化最小的推荐点击 action/source，再立即导航；下一预测机会的模型状态读取、特征准备和推理异步执行，不得阻塞导航关键路径。气泡按压 indication 必须由同一圆角 Surface 裁剪。
...
- 只在登录有效、当前 profile/card/generation 匹配、前台、网络安全状态正常、预测概率和收益门槛达标时请求。
...
- 气泡可在多页面安全展示，满足频率、冷却、路由和遮挡约束。
- 同时只有一个气泡；路由切换、弹窗、键盘、付款码和交易页正确避让。
- 点击、关闭、超时、TalkBack 和“不再推荐”正确。
...
| 气泡过于频繁 | 打扰和关闭率上升 | 全局单实例、分钟/session/日频控、action 冷却 |
...
- 智能预加载默认开启，完整动作目录中有安全资源的动作使用 TTL、single-flight 和预算。
