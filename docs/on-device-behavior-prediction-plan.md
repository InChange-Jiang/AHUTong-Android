...
- suggestion state 显式携带基于 `elapsedRealtime` 的显示与到期时刻。气泡在 12 秒 TTL 内按剩余时间比例从完全可见线性淡出，到期由运行时撤销；重组或配置变化后从真实剩余比例继续，不能无故重新变回不透明。用户按下气泡时必须取消旧的运行时到期任务、把透明度恢复为 100% 并在按住期间暂停倒计时；松开或手势取消后写入新的显示/到期时刻，以完整 12 秒 TTL 重新开始淡出和消失计时。超时任务使用 generation 校验，已取消的旧任务不得在竞态中隐藏新窗口。
...
- 主体点击即接受建议。点击必须先耐久化最小的推荐点击 action/source，再立即导航；下一预测机会的模型状态读取、特征准备和推理异步执行，不得阻塞导航关键路径。气泡按压反馈复用底部液态玻璃控件的 `InteractiveHighlight`：高光跟随触点，玻璃胶囊随按压和拖动产生受限的缩放、位移与方向形变，松开或取消后按同一弹簧参数回弹。气泡使用 0.9 倍最短边的局部高光半径，底部 Navigation 继续使用原有 1.5 倍半径，避免气泡内出现过大的白色光圈。动画作用域只负责视觉反馈；点击导航仍同步移交 `Main` 持有的父级作用域，不能重新放回气泡生命周期作用域。
